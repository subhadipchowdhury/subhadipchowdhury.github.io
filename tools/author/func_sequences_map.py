...
Fourteen boxes and fifteen arrows, with the tikz coordinates carried over. Every
arrow holds in the direction drawn; arrow 6, the Cauchy criterion against uniform
convergence, is the one that runs both ways.

Arrows 2 and 5 were failures until 2026-08-13. Arrow 2 was "pointwise convergence
does not preserve continuity" and now runs A to G, naming the three hypotheses Dini
adds to close exactly that gap. Arrow 5 was "uniform convergence does not give a
...
DATA = {
    ...
    "intro": (
        "Fourteen ideas from the chapter on sequences and series of functions, and "
        "fifteen arrows between them that we haven't drawn. Every arrow is a "
        "relationship of some kind: a few are implications between properties, some "
    ...
    "reflection": (
        ...
        "the two kinds of convergence, and most of the chapter follows from where it "
        "lands, so say whether your answer is the arrow that moves it."
    ),
    "nodes": NODES,
    "edges": EDGES,
    "mistakes": MISTAKES,
