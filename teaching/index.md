...
<div class="course">
{{ course.Role }} for **{{ course.Name }} (Math {{ course.Code }})**, taught by Professor {% if course.Homepage %}[{{ course.Instructor }}]({{ course.Homepage }}){% else %}{{ course.Instructor }}{% endif %} - {% if course.Link %}[{{ course.Duration }}]({{ course.Link }}){% else %}{{ course.Duration }}{% endif %}
